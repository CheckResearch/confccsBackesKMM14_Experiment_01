...
FamilyFingerprintMap *Graph::constructFamilyFingerprintMapAndReadMoreInfo(const char* server_descriptor_db_filename)
{
	...
	} else
	{
		...
		for (QueryResult::iterator it = result_vector->begin(); it != result_vector->end(); ++it)
		{
			...
			result_map->insert(make_pair<Hash,pair<string, Row> >(hash, make_pair<string,Row>(it->at(1),family_fingerprints)));
		}
...
void Graph::assignFamilies()
{
	...
	for (unsigned int i=0; i < this->size(); i++)
	{
		...
		if (res != this->family_fingerprint_map->end())
		{
			...
			fingerprint_node_map.insert(make_pair<Hash,Node*>(current_node->getFingerprint(),current_node));
			nickname_node_map.insert(make_pair<Hash,Node*>(current_node->getName(),current_node));
		}
	}
